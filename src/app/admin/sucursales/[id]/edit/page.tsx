...
export default function EditBranchPage() {
  ...
  const removeState = (stateToRemove: string) => {
    const checkStateInUse = async () => {
      // Verificar si hay otras sucursales (excluyendo la actual) usando este estado
      const { data, error } = await supabase
        .from('branches')
        .select('id, name')
      ...
      if (data && data.length > 0) {
        toast.error(`No se puede eliminar el estado "${stateToRemove}" porque hay otras sucursales que lo están usando`)
        return false
      }
    ...
    checkStateInUse().then(canRemove => {
      if (canRemove) {
        ...
        setAvailableStates(updated)
        // Solo limpiar si el estado eliminado es el seleccionado y no es el de esta sucursal actual
        if (state === stateToRemove && branch && branch.state !== stateToRemove) {
          setState('')
        }
  ...
  const fetchBranch = async () => {
    try {
      ...
      setHorarios(data.horarios && data.horarios.length > 0 ? data.horarios : [''])
      setPrices(data.prices && Array.isArray(data.prices) && data.prices.length > 0 ? data.prices : [{ title: '', price: '', description: '', image: '' }])
      setAttractions(data.attractions && Array.isArray(data.attractions) && data.attractions.length > 0 ? data.attractions : [{ name: '', image: '' }])
      setGallery(data.gallery && Array.isArray(data.gallery) && data.gallery.length > 0 ? data.gallery : [{ src: '', alt: '' }])
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center space-x-4 py-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Actualiza los detalles de la sucursal</p>
            </div>
          </div>
        </div>
      </header>
