  ...
  return (
    <>
      ...
      <div className="fixed top-4 right-4 z-50 hidden md:flex">
        <div className={cn("flex items-center justify-center bg-accent/80 backdrop-blur-lg rounded-full shadow-2xl transition-all duration-700 ease-in-out", !navVisible && "w-11 h-11", navVisible && "p-2")}>
          <nav
            className={cn(
            ...
            <div className={cn("flex items-center gap-4 whitespace-nowrap", navVisible ? "px-4" : "px-0")}>
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm font-medium text-background transition-colors hover:text-background/80"
                >
                  {link.label}
                </Link>
              ))}
      ...
      <div className="fixed top-4 right-4 z-50 md:hidden">
        <div className="flex items-center gap-2">
            {socialLinks.map((social) => (
                <Button key={social.label} variant="ghost" size="icon" asChild className="rounded-full bg-accent/90 backdrop-blur-xl shadow-2xl text-background w-10 h-10 hover:bg-background/20">
            ...
            <Sheet>
                ...
                <Button variant="ghost" size="icon" className="bg-accent/90 backdrop-blur-xl rounded-full w-10 h-10 shadow-2xl text-background hover:bg-background/20">
                    <Menu className="h-6 w-6" />
                    <span className="sr-only">Toggle navigation menu</span>
                </Button>
                </SheetTrigger>
                <SheetContent side="right" className="bg-accent/90 backdrop-blur-xl border-l-0">
                <div className="flex flex-col gap-6 pt-12 h-full">
                    <div className="flex flex-col gap-6">
                        ...
                        <SheetClose asChild key={link.href}>
                            <Link
                            href={link.href}
                            className="text-2xl font-medium text-background transition-colors hover:text-background/80"
                            >
                            {link.label}
                            </Link>
                        </SheetClose>
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex gap-4">
                            {socialLinks.map((social) => (
                                <Button key={social.label} variant="ghost" size="icon" asChild className="rounded-full bg-background/20 backdrop-blur-sm text-accent hover:bg-accent/80">
                                <Link href={social.href} aria-label={social.label}>
                                    {social.icon}
                                </Link>
                                </Button>
